...
export function WsProvider({
  ...
  useEffect(() => {
    Object.keys(config.cameras).forEach((camera) => {
      const { name, record, detect, snapshots, close_contacts } = config.cameras[camera];
      ...
      dispatch({ topic: `${name}/close_contacts/state`, payload: close_contacts.enabled ? 'ON' : 'OFF', retain: false });
    });
  }, [config]);
...
export function useCloseContactsState(camera) {
  const {
    value: { payload },
    send,
    connected,
  } = useWs(`${camera}/close_contacts/state`, `${camera}/close_contacts/set`);
  return { payload, send, connected };
}
